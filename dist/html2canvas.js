...
function getDocWidth(node, isDGLux) {
  ...
  var hasScroll = !isDGLux || node.ownerDocument.body.children[0].style.overflowX !== 'hidden' && (!dgPage || dgPage.style.overflowX != 'hidden');
  ...
  if (hasScroll) {
    arr = arr.concat([node.scrollWidth, node.ownerDocument.documentElement.scrollWidth]);
  }

  return Math.max.apply(this, arr.filter(function (a) {
    return !!a;
  }));
}
...
function getDocHeight(node, isDGLux) {
  ...
  var hasScroll = !isDGLux || node.ownerDocument.body.children[0].style.overflowY !== 'hidden' && (!dgPage || dgPage.style.overflowY != 'hidden');
  ...
  if (hasScroll) {
    arr = arr.concat([node.scrollHeight, node.ownerDocument.documentElement.scrollHeight]);
  }
...
function html2canvas(nodeList, options) {
  ...
  if (options.isDGLux) {
    ...
    if (!!dgPage && dgPage.style.overflowY == "scroll" && dgPage.scrollWidth > dgPage.clientWidth) {
      var _ratio = dgPage.clientWidth / dgPage.scrollWidth;
      Array.prototype.slice.call(dgPage.children).forEach(function (childNode) {
        if (childNode.style.width.indexOf("%") > 0) {
          childNode.style.width = Math.round(width * _ratio * parseInt(childNode.style.width) / 100) + "px";
        }
      });
    }
  }
